    ...
    (when (stringp path)
      (let ((found-paths (sort (loop for re in (lazy-buildsystem-patterns)
        ...
        (if (car found-paths)
          ...
          (let ((next-path (butlast (split-string path "/"))))
            (when (> (length next-path) 0)
              (lazy-guess-basedir-from-buildsystem (print (mapconcat 'identity next-path "/"))))
            ))))))
