...
`lazy-compile-next-history-element' and `lazy-compile-previous-history-element'."
  ...
  (if (and (called-interactively-p 'interactive)
    ...
    (let* ((cmd (lazy-get-config-val 'compile-cmd))
      ...
      (when (string-equal (buffer-name buf) "*compilation*")
        (call-interactively 'other-window)
        (setq buf (current-buffer)))
      (let ((buffers (lazy-buffers)))
        (while (and (not (buffer-file-name buf))
                    (> (length buffers) 0))
          (setq buf (pop buffers))))
      (lazy-with-directory (cond ((setq friend (lazy-friendly-buffer-p buf))
                                  (or (lazy-get-config-val 'basedir friend)
                                      (when (file-directory-p friend) friend)
                                      (file-name-directory (buffer-file-name buf))))
                                 ((lazy-buffer-p buf)
                                  (lazy-get-config-val 'basedir))
                                 (t
                                  (file-name-directory (buffer-file-name buf))))
                           (cond ((listp cmd)
                                  (let* ((old-history cmd)
...
(defun lazy-make-temporary-friend-alist (friend-dir proj-name proj-alist)
  (let* ((friend-basedir (expand-file-name friend-dir))
         ...
         (friend-ignore-rg (lazy-get-config-val 'ignore-rg proj-name t proj-alist))
    ...
    (list (list 'name friend-name)
          ...
          (list 'ignore-rg friend-ignore-rg)
