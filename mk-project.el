...
(defun mk-proj-jump-list (system &rest args)
  (message "get-jumps")
...
;;(mk-proj-jump-list 'gtags "global -x -e \"pr.*\"")
...
(defun mk-proj-score-jumps (jumps regexp buffer)
  ...
  (let ((file-map (make-hash-table :test 'equal))
        ...
        (basedir-path (file-truename (or (mk-proj-get-config-val 'basedir nil t nil (mk-proj-guess-alist))
                                         default-directory)))
    ...
    (dolist (jump jumps)
      (let* ((jump-word (plist-get jump :word))
             (jump-relative-path (plist-get jump :relative-path))
             (jump-path (or (gethash jump-relative-path truenames-cache) (puthash jump-relative-path (file-truename jump-relative-path) truenames-cache)))
...
(defun mk-proj-compare-jumps (a b)
  (let ((file-score-a (plist-get a :file-score))
        ...
        (file-name-a (file-name-base (plist-get a :relative-path)))
        (file-name-b (file-name-base (plist-get b :relative-path)))
        ...
        (with-current-buffer (get-buffer-create mk-proj-jump-buffer)
          ...
          (let ((id 0))
            (dolist (jump (mk-proj-sort-jumps jump-list))
              (let ((relative-path (plist-get jump :relative-path))
                    (line-number (plist-get jump :line-number))
                ...
                (push (list id (vector (list word
                                       ...
                                       (list (concat (file-name-nondirectory relative-path) ":" (format "%d" line-number))
                                             'mouse-face 'highlight
    ...
    (when (or (not buffer-systems)
      ...
      (let ((jump-map (make-hash-table :test 'equal))
        ...
        (mapc (lambda (jump)
              ...
              (mk-proj-jump-list 'gtags (concat "global --match-part first -c " (prin1-to-string prefix))))
...
(defun project-jump-definition (&optional word buffer)
  ...
  (unless buffer
    (setq buffer (current-buffer)))
  (unless word 
    (setq word (ido-completing-read "Symbol: " (mk-proj-completions) nil t (substring-no-properties (or (thing-at-point mk-proj-thing-selector) "")))))
  (let ((buffer-systems (mk-proj-src-pattern-tag-systems (when (buffer-file-name buffer) (list (buffer-file-name buffer)))))
        (jumps '()))
    (when (or (not buffer-systems)
              (find 'gtags buffer-systems)
              (find 'gtags+rtags buffer-systems)
              (find 'gtags+exuberant-ctags buffer-systems))
      (setq jumps (nconc jumps (mk-proj-jump-list 'gtags (concat "global -x -d " (prin1-to-string word))))))
    (when (or (not buffer-systems)
...
(defun project-jump-regexp (regexp &optional buffer)
  ...
  (let ((buffer-systems (mk-proj-src-pattern-tag-systems (when (buffer-file-name buffer) (list (buffer-file-name buffer)))))
    ...
    (when (or (not buffer-systems)
              ...
              (find 'gtags+rtags buffer-systems)
              (find 'gtags+exuberant-ctags buffer-systems))
      (setq jumps (nconc jumps (mk-proj-jump-list 'gtags (concat "global -x -e " (prin1-to-string regexp))))))
    (when (or (not buffer-systems)
              (find 'rtags buffer-systems)
              (find 'gtags+rtags buffer-systems))
      nil)
