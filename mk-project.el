      ...
      (dolist (nfl not-found1)
        (let* ((m (first nfl))
          ...
          (while (re-search-forward (regexp-quote l) nil t)
            (setq matches (cons (point-at-bol) matches)))
          (cond ((> (length matches) 1)
                 (setq tm (cons (list m matches) tm)))
                ((= (length matches) 1)
              ...
              (setq znf3 (mapcar (lambda (zs)
                                   (if (fifth zs)
                                       (let* ((m (first zs))
                                         ...
                                         (cond ((> thisfuzz (- (length string) threshold))
          ...
          (dolist (zs znf3)
            (let ((m (first zs))
              ...
              (cond ((= (length matches) 1)
                     (progn
                       (goto-char 0)
                       (when (re-search-forward (regexp-quote (car matches)) nil t)
                         (setq rs (cons (point-at-bol) rs)))))
                    ((> (length matches) 1)
                     (setq tm (cons (list m matches) tm))))))))
      (unless (null tm)
        )
...
;; (mk-proj-sourcemarker-fuzzy-search (make-list 50 mk-proj-sourcemarker))
